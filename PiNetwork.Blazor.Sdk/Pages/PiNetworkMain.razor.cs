...
namespace PiNetwork.Blazor.Sdk.Pages
{
    public partial class PiNetworkMain : ComponentBase
    {
        ...
        [JSInvokable]
        public async Task AuthenticateOnSuccess(AuthResultDto auth, string redirectUri)
        {
            if (this.logger is { })
                this.logger.LogInformation("Method: {@Method}. Authentication: {@Authentication}. RedirectUri: {RedirectUri}", nameof(AuthenticateOnSuccess), auth, redirectUri);

            await this.sdk.AuthenticateOnSuccessCallBack(auth, redirectUri);
        }
        ...
        public async void AuthenticateOnError(string error, string redirectUri)
        {
            ...
            await this.sdk.AuthenticateOnErrorCallBack(error, redirectUri);
        }
        ...
        public async Task IsPiNetworkBrowser()
        {
            ...
            */
            await this.sessionStorage.SetItemAsStringAsync(ConstantsEnums.ConstantsEnums.IsPiNetworkBrowser, "1");
        }

        [JSInvokable]
        public static Task<object> Test(object item)
        {
            return Task.FromResult(item);
        }

        [JSInvokable]
        public async Task Test2(object item)
        {
            if (this.logger is { })
                this.logger.LogInformation("Method: {@Method}. I work properly with logger", nameof(Test2));
